...
**경로 위험 비교.** 미국 내 출발지와 도착지를 입력하면 세 가지 대안을 보여줍니다. 가장 빠른 길, 기상 위험이 낮은 길, 그리고 그 둘의 균형입니다. 각 경로에 구간별 리스크 설명이 붙습니다. "왜 이 코리도어(경로 축을 이루는 주요 도로 구간)가 더 위험한지" 화면에서 직접 확인할 수 있습니다.

**실시간 위험 정보.** 전국 기상 위험 전망을 제공하고, 지점별 리스크 점수를 계산하고, 위험 유형·도시·카운티·코리도어 기준으로 경보를 검색합니다. 데이터 소스는 NWS와 NOAA입니다. 소스가 내려가면 가짜 수치를 채워 넣지 않고 "현재 이용 불가"로 표시합니다.

**경로 워치리스트.** 로그인한 사용자는 경로를 저장하고, 위험 임계값을 설정하고, 모니터링을 활성화하고, 리스크를 직접 갱신하고, 과거 위험 이력을 조회할 수 있습니다. 저장 데이터는 전부 DynamoDB에서 관리합니다. 쓰기 조건에 소유자 범위를 포함하므로 다른 사용자의 데이터에 접근할 수 없습니다.

**SaaS 계정 레이어.** 플랜(FREE/PRO/TEAM/INTERNAL)별로 일일 사용량을 제한하고, 저장 경로와 장소 용량을 관리하고, 쿼터 초과 시 구조화된 에러를 반환합니다. 멱등성(같은 요청을 여러 번 보내도 결과가 동일한 성질) 키로 안전 재시도도 지원합니다. 결제 연동은 제외했습니다. 권한 경계를 먼저 확립하면 향후 결제 통합이 깔끔해집니다.

**다중 경유지 최적화.** VRP(Vehicle Routing Problem, 다중 경유지 최적화 문제) 기반입니다. OR-Tools 솔버 위에 리스크 가중 엣지 코스트를 올렸습니다. ML 섀도 워크플로우는 프로모션 게이트를 전부 통과해야만 솔버 코스트에 반영됩니다.

**운영 상태 투명 공개.** `/status` 페이지에서 데이터 소스 생존 여부를 확인하고, 프론트엔드 재시도·폴백 횟수를 추적하고, 브라우저 성능 스냅샷과 세션 범위 에러 로그를 노출합니다. 서드파티 분석 도구는 사용하지 않습니다.
...
React 19 SPA를 Private S3에 올리고 CloudFront로 서빙합니다. API 호출은 API Gateway를 거쳐 백엔드 두 개로 라우팅합니다. Spring Boot가 인증을 처리하고, 테넌트를 식별하고, 권한을 검사하고, 사용자 데이터를 저장합니다. FastAPI가 라우팅을 수행하고, 기상을 스코어링하고, 경보를 집계하고, VRP 최적화를 실행합니다. 운영 스토어는 DynamoDB입니다(유휴 비용 0). 공간 조인이 필요해지면 PostGIS로 확장하며, 이 경로는 ADR로 문서화했습니다.
...
카테고리 점수를 단순 평균하면 문제가 생깁니다. 토네이도 경보가 하나 떠 있어도 "전체적으로 맑음"이면 점수가 낮게 나옵니다. 대신 `max`를 취합니다.
...
`max`을 취하면 심각한 경보 하나가 전체 점수를 지배합니다. 반대로 경보 없이 비, 바람, 더위가 겹치는 경우는 가중 합성이 잡아줍니다. 전국 리스크는 상위 20개 경보만 평균냅니다. 수백 개 저심각도 자문문이 점수를 희석하지 않습니다.

전국 엔드포인트에는 60초 TTL(Time-To-Live, 캐시 만료 시간) 인메모리 캐시를 적용했습니다. 동시 요청 시 NWS API 반복 호출을 방지합니다.

검증: `test_api.py`, `test_hazards.py`, `test_weather_snapshot.py`

### 경로 대안 비교: 라벨이 붙는 과정

OSRM에 `alternatives=3`을 요청하면 기하학적으로 서로 다른 경로를 받아옵니다. 각 후보 경로에서 웨이포인트를 찍어 기상을 샘플링합니다. NWS 경보 지오메트리와 교차하는지 확인하여 점수를 매깁니다. 그 다음 소요시간순으로 정렬합니다:
...
리스크 엔진은 NWS, Open-Meteo, OSRM, WZDx 같은 외부 API를 호출합니다. 임의 URL을 허용하면 SSRF(Server-Side Request Forgery, 서버 측 요청 위조) 공격면이 생깁니다. `outbound_http.py`에 게이트를 두고 모든 아웃바운드 요청이 통과하도록 설계했습니다:

1. **호스트 허용 목록.** api.weather.gov, router.project-osrm.org 등 사전 승인 도메인만 통과합니다. 환경변수로 명시적으로 추가할 수 있습니다.
2. **HTTPS만.** 공개 호스트에 평문 HTTP 요청은 즉시 거부합니다.
3. **리다이렉트 차단.** 커스텀 `HTTPRedirectHandler`가 리다이렉트 시 예외를 발생시킵니다. 오픈 리다이렉트를 통한 허용 목록 우회 공격을 차단합니다.
4. **DNS 해석 검증.** 연결 전에 호스트네임을 해석하여 loopback, link-local, private, reserved, multicast에 해당하면 차단합니다. DNS 리바인딩으로 `169.254.169.254`(클라우드 메타데이터)를 찌르는 공격을 막는 핵심 장치입니다.
5. **자격 증명 제거.** URL에 `user:pass@`가 포함되어 있으면 거부합니다.

로컬 개발 시 localhost 접근이 필요하면 `ATMOSPATH_ALLOW_LOCAL_OUTBOUND=true`를 명시적으로 설정합니다.
...
1. 키 형식을 검증합니다 (1~128자, `[A-Za-z0-9._:-]`만 허용).
2. **저장 전에 SHA-256으로 해싱합니다.** 원본 클라이언트 키가 DynamoDB에 그대로 들어가지 않습니다.
...
VRP 지연 모델은 솔버 코스트에 영향을 줄 수 있습니다. 그러나 기본은 섀도 모드입니다. 룰 기반 점수가 권위이며, ML이 코스트를 바꾸려면 게이트 4개를 전부 통과해야 합니다:

1. **학습.** 모델 학습 + 평균 지연 baseline 대비 백테스트. MAE가 baseline보다 개선되어야 릴리스 게이트를 통과합니다.
2. **프로모션.** CLI로 `served_to_users=true` 아티팩트를 새로 생성합니다. 섀도 아티팩트는 계속 `false`입니다.
3. **런타임.** 환경변수에 `VRP_ML_WORKFLOW_MODE=SERVING_ENABLED` + `VRP_ML_ALLOW_SERVED_COST=true`가 필요합니다.
4. **요청.** 개별 요청에서 `useMlServedCost=true`를 명시적으로 전송해야 합니다.

하나라도 빠지면 룰 기반 코스트로 fail-closed(조건 미충족 시 기본값으로 복귀) 처리합니다. 반영되는 지연도 상한(`mlMaxDelaySeconds`), 가중치(0.35), 신뢰도 임계값으로 한 번 더 걸러집니다. 아티팩트는 pickle이 아닌 순수 JSON(계수, 피처명, 메트릭)입니다. 사람이 열어볼 수 있고, 로드 시 임의 코드가 실행되지 않습니다.
...
- **재시도.** 5xx나 네트워크 에러만 백오프(점진적 대기 간격 증가)로 재시도합니다. 4xx는 재시도하지 않습니다.
- **Stale 캐시 폴백.** 공개 리스크 응답을 `sessionStorage`(512 kB 상한)에 캐시합니다. 새 요청이 실패하면 캐시된 데이터와 "직전 데이터입니다" 배너, 시각을 함께 표시합니다.
...
DynamoDB 온디맨드는 유휴 비용이 0입니다. 소유자 범위 조회는 한 자릿수 ms로 처리하고, 조건부 쓰기로 낙관적 동시성을 확보합니다. 공간 쿼리는 불가하므로 PostGIS 확장 경로로 분리했습니다 (ADR-006).
...
승수는 6, 6, 8, 10으로 다릅니다. 홍수(8)와 경보(10)는 "불쾌한 수준"이 아니라 "도로가 막히는 수준"과 상관하므로 더 무겁게 설정했습니다. 행렬에서 엣지가 빠진 경우(경로 없음)는 24시간 페널티를 부여하여 라우팅에서 제외합니다. ML 섀도 모델이 활성화되어 있으면 이 기본 코스트 위에 신뢰도 게이트 지연이 추가됩니다.
...
`risk.py`(28%)와 `directions.py`(54%)는 커버리지가 낮습니다. 외부 API(NWS, NOAA, OSRM) 호출 경로를 단위 테스트에서 제외했기 때문입니다. 이 경로들은 통합 테스트와 E2E에서 간접적으로 검증합니다. 단위 테스트에서는 비즈니스 로직과 데이터 변환에 집중합니다.
...
목업이 아닌 실행 중인 앱을 Playwright로 캡처했습니다. 재생성: `npm run screenshots:release --prefix web`.
...
개별 레이어만 실행할 경우 Node 20+, Python 3.12+, Java 21이 필요합니다. Windows에서는 `.tools/` 디렉터리에 Java 21과 Maven이 번들되어 있습니다. 별도 설치 없이 바로 실행할 수 있습니다.
...
인증은 Cognito JWT를 사용합니다. Google OAuth 페더레이션 경로는 문서화했습니다. 공개 지도 API는 로그인 없이 사용하되 서버에서 레이트 리밋을 적용합니다. `/me/**`는 유효 토큰이 없으면 401을 반환합니다.

핵심 하드닝:

- DynamoDB 파티션 키를 소유자 범위로 설정하고 조건부 쓰기/삭제를 적용합니다. 다른 테넌트 데이터에 접근할 수 있는 경로 자체가 없습니다.
- 멱등성 키는 SHA-256 해시 후 테넌트 범위로 저장합니다. 원본 키는 어디에도 남지 않습니다.
- Spring API에 고정 윈도우 레이트 리밋을 적용합니다. 인메모리가 기본이며, Redis로 전환할 수 있습니다.
- CloudFront에서 오리진을 검증(`X-Origin-Verify`)하고, CSP, frame-deny, referrer policy를 적용합니다.
- 아웃바운드 HTTP에 허용 목록, HTTPS, 리다이렉트 차단, 메타데이터/사설 네트워크 거부를 적용합니다.
- 배포는 GitHub Actions OIDC로 수행합니다. 장기 AWS 액세스 키는 이 프로젝트 어디에도 없습니다.
- 프론트엔드 E2E에 지도 마커 XSS, 인증 불가 시 메시징, stale 데이터 폴백 시나리오를 포함합니다.

---

## ML 워크플로우

기본은 섀도 모드입니다. 룰 기반 점수가 권위이며, ML이 코스트를 바꾸려면 다음 단계를 전부 통과해야 합니다:
...
하나라도 충족하지 않으면 룰 기반 코스트로 fail-closed 처리합니다. 반영되는 지연에도 상한, 가중치, 신뢰도 임계값을 적용합니다.
...
**로컬 스트레스 테스트** (외부 호출 없음, 인프로세스): 180 요청 / 동시성 8 / 실패 0건. 캐시 적용 엔드포인트 p95가 500 ms 이하입니다. 다중 경유지 계획 p95 ~3.2 s가 현재 보틀넥입니다. 동기 행렬 연산이 원인이며, 비동기 잡으로 분리하는 것이 다음 단계입니다.
...
AWS `us-east-1`, 서버리스 우선입니다. CloudFront + Private S3, API Gateway + Lambda, DynamoDB 온디맨드, Cognito로 구성합니다. Kubernetes, Aurora, 매니지드 Redis는 사용하지 않습니다.

인프라는 전부 Terraform으로 관리합니다. CI/CD는 GitHub Actions OIDC(정적 자격 증명 없음)로 운영합니다. CloudFront에 US/KR 지역 제한을 적용했습니다. 리소스 태그는 `Project=awsresumeproject`, `ManagedBy=IaC`, `Environment=dev`로 통일했습니다.

[배포 문서](docs/deployment.md)와 [비용 모델](docs/cost-model.md)에서 자세한 내용을 확인합니다.
