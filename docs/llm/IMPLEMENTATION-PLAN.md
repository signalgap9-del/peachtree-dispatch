...
| Phase | 줄 수 | 핵심 산출물 |
|---|---|---|
| 1. LLM 인프라 | ~2,000 | LiteLLM + Langfuse + Spring 오케스트레이터 |
| 2. NL2Opt 코어 | ~3,500 | 스키마, 추출/포뮬레이션/해석 에이전트, 멀티턴 |
| 3. 벡터 검색 + RAG | ~3,500 | pgvector, 하이브리드 검색, 리랭킹, 평가 |
| 4. 능동 인텔리전스 | ~1,500 | SSE + LLM 판단 + 능동 제안 |
| 5. 프론트엔드 | ~2,500 | 채팅 UI, 스트리밍, 지도 연동 |
| 6. 안전 + 평가 + 문서 | ~1,500 | 가드레일, 벤치마크, ADR |
| **합계** | **~16,500** | |
